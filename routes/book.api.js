...
router.delete("/:bookId", (req, res, next) => {
  // -------------------- Step 1: delete input validation
  try {
    const { bookId } = req.params;
    // -------------------- Step 2: delete processing logic
    //Read data from db.json then parse to JSobject
    let db = fs.readFileSync("db.json", "utf-8");
    db = JSON.parse(db);
    const { books } = db;
    ...
    //filter db books object (this means remove the matched item from db)
    db.books = books.filter((book) => book.id !== bookId);
    //db JSobject to JSON string
    db = JSON.stringify(db);
    //write it back and save to db.json
    fs.writeFileSync("db.json", db);
    // -------------------- Step 3: delete send response
    res.status(200).send({});
  } catch (error) {
    next(error);
  }
